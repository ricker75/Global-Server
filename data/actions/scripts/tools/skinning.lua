...
local random = math.random(1,10)
        ...
        return true
        elseif isInArray(rabbit, itemEx.itemid) then
                if (getPlayerStorageValue(cid, 41600) == 1) and getPlayerStorageValue(cid, 374325) <= os.time() then
                        doPlayerAddItem(cid, 13159, 1)
			doCreatureSay(cid, "You successfully gathered a rabbit's food in excellent condition.", TALKTYPE_ORANGE_1)
			setPlayerStorageValue(cid, 374325, os.time()+10)
		end
  	 return true
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	--- The Elemental Spheres Quest by MaXwEllDeN
	local itEx = Item(itemEx.uid)
	local p = Player(cid)
	if item.itemid == 5908 and isInArray({9009, 9010}, itemEx.itemid) and p:getStorageValue(70000) < 1 then
		p:addItem(8310, 1)
		toPosition:sendMagicEffect(13)
		p:setStorageValue(70000, 1)
		return true
	end
