...
function greetCallback(cid)
	...
	if player:getStorageValue(Storage.spikeTask.Undercover) > 0 and player:getStorageValue(Storage.spikeTask.Undercover) < 4 and player:getStorageValue(Storage.spikeTask.Behemoth) <= os.time() then
		...
		player:setStorageValue(Storage.spikeTask.Behemoth, os.time() + 20*3600)
	else
