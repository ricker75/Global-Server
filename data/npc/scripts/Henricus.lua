...
local function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "mission") or msgcontains(msg, "report")) then
		if(player:getStorageValue(200) == 1) then
			selfSay("Let's see if you are worthy. Take an inquisitor's field guide from the box in the back room. ...", cid)
		...
		elseif(player:getStorageValue(200) == 3) then
			selfSay("Listen, we have information about a heretic coven that hides in a mountain called the Big Old One. The witches reach this cursed place on flying brooms and think they are safe there. ...", cid)
			selfSay("I've arranged a flying carpet that will bring you to their hideout. Travel to Femor Hills and tell the carpet pilot the codeword 'eclipse' ...", cid)
			selfSay("He'll bring you to your destination. At their meeting place, you'll find a cauldron in which they cook some forbidden brew ...", cid)
		...
		elseif(player:getStorageValue(200) == 5) then
			if (player:removeItem(8702, 1)) then
				selfSay("I think it's time to truly test your abilities. One of our allies has requested assistance. I think you are just the right person to help him ...", cid)
				selfSay("Storkus is an old and grumpy dwarf who works as a vampire hunter since many, many decades. He's quite successful but even hehas his limits. ...", cid)
				selfSay("So occasionally, we send him help. In return he trains and tests our recruits. It's an advantageous agreement for both sides ...", cid)
				npcHandler:say("You'll find him in his cave at the mountain outside of Kazordoon. He'll tell you about your next mission.", cid)
				npcHandler.topic[cid] = 0
		...
		elseif(player:getStorageValue(200) == 11) then
			selfSay("We've got a report about an abandoned and haunted house in Liberty Bay. I want you to examine this house. It's the only ruin in Liberty Bay so you should have no trouble finding it. ...", cid)
			npcHandler:say("There's an evil being somewhere. I assume that it will be easier to find the right spot at night. Use this vial of holy water on that spot to drive out the evil being.", cid)
		...
		elseif(player:getStorageValue(200) == 14) then
			selfSay("You've handled heretics, witches, vampires and ghosts. Now be prepared to face the most evil creatures we are fighting - demons. Your new task is extremely simple, though far from easy. ...", cid)
		...
		elseif(player:getStorageValue(200) == 15) then
			if(player:getItemCount(6500) >= 20) then
				selfSay("You're indeed a dedicated protector of the true believers. Don't stop now. Kill as many of these creatures as you can. ...", cid)
		...
		elseif(player:getStorageValue(200) == 17) then
			selfSay("We've got information about something very dangerous going on on the isle of Edron. The demons are preparing something there ...", cid)
			selfSay("Something that is a threat to all of us. Our investigators were able to acquire vital information before some of them were slain by a demon named Ungreez. ...", cid)
			npcHandler:say("It'll be your task to take revenge and to kill that demon. You'll find him in the depths of Edron. Good luck.", cid)
			npcHandler.topic[cid] = 0
		...
		elseif(player:getStorageValue(200) == 19) then
			selfSay("So the beast is finally dead! Thank the gods. At least some things work out in our favour ...", cid)
			npcHandler:say("Our other operatives were not that lucky, though. But you will learn more about that in your next {mission}.", cid)
			npcHandler.topic[cid] = 0
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif(npcHandler.topic[cid] == 3) then
			if(player:getStorageValue(201) == 1 and player:getStorageValue(202) == 1 and player:getStorageValue(203) == 1 and player:getStorageValue(204) == 1 and player:getStorageValue(205) == 1) then
				selfSay("Indeed, this is exactly what my other sources told me. Of course I knew the outcome of this investigation in advance. This was just a test. ...", cid) 
		...
		elseif(npcHandler.topic[cid] == 6) then
			if (player:getStorageValue(200) == 22) then
				selfSay("Incredible! You're a true defender of faith! I grant you the title of a High Inquisitor for your noble deeds. From now on you can obtain the blessing of the inquisition which makes the pilgrimage of ashes obsolete ...", cid)
