...
function greetCallback(cid)
	...
	if player:getStorageValue(Storage.spikeTask.Undercover) > 0 and player:getStorageValue(Storage.spikeTask.Undercover) < 4 and player:getStorageValue(Storage.spikeTask.Wyrm) <= os.time() then
		...
		player:setStorageValue(Storage.spikeTask.Wyrm, os.time() + 20*3600)
	else
