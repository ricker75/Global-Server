...
local function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, 'mission') or msgcontains(msg, 'demon oak')) then
		if(player:getStorageValue(1010) < 1 and player:getStorageValue(1013) < 2) then
			npcHandler:say("How do you know? Did you go into the infested area?", cid)
			npcHandler.topic[cid] = 1
		elseif (player:getStorageValue(1013) == 2 and player:getStorageValue(1010) < 1) then
			npcHandler:say("You better don't return here until you've defeated the Demon Oak.", cid)
		elseif (player:getStorageValue(1010) == 1) then
			selfSay("You chopped down the demon oak?!? Unbelievable!! Let's hope it doesn't come back. As long as evil is still existent in the soil of the plains, it won't be over. Still, the demons suffered a setback, that's for sure. ...", cid)
			selfSay("For your brave action, I tell you a secret which has been kept for many many years. There is an old house south of the location where you found the demon oak. There should be a grave with the name 'Yesim Adeit' somewhere close by. ...", cid)
			...
			player:setStorageValue(1010, 2)
		end
	elseif (msgcontains(msg, 'yes') and npcHandler.topic[cid] == 1) then
		if (player:getStorageValue(1013) == 1) then
	...
	elseif (msgcontains(msg, 'axe') or msgcontains(msg, 'hallowed axe')) then
		if (player:getStorageValue(1013) == 2) then
