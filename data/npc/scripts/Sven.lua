...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "test") then
		if npcHandler.topic[cid] == 1 then
			selfSay("All of our juveniles have to take the barbarian test to become a true member of our community. Foreigners who manage to master the test are granted the title of an honorary barbarian and the respect of our people ...", cid)
	...
	elseif msgcontains(msg, "mead") then
		...
		elseif player:getStorageValue(120) == 3 then
			selfSay("An impressive start. Here, take your own mead horn to fill it at the mead bucket as often as you like ...", cid)
			selfSay("But there is much left to be done. Your next test will be to hug a bear ...", cid)
			selfSay("You will find one in a cave north of the town. If you are lucky, it's still sleeping. If not ... well that might hurt ...", cid)
	...
	elseif msgcontains(msg, "hug") then
		if player:getStorageValue(120) == 5 then
			selfSay("Amazing. That was as clever and brave as a barbarian is supposed to be. But a barbarian also has to be strong and fearless. To prove that you will have to knock over a mammoth ...", cid)
			selfSay("Did your face just turn into the color of fresh snow? However, you will find a lonely mammoth north west of the town in the wilderness. Knock it over to prove to be a true barbarian ...", cid)
	...
	elseif msgcontains(msg, "mammoth") then
		if player:getStorageValue(120) == 7 then
			selfSay("As you have passed all three tests, I welcome you in our town as an honorary barbarian. You can now become a citizen. Don't forget to talk to the people here. Some of them might need some help ...", cid)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 2 then
			selfSay("That's the spirit! The barbarian test consists of a few tasks you will have to fulfill. All are rather simple - for a barbarian that is...", cid)
			selfSay("Your first task is to drink some barbarian mead. But be warned, it's a strong brew that could even knock out a bear. You need to make at least ten sips of mead in a row without passing out to pass the test ...", cid)
			npcHandler:say("Do you think you can do this?", cid)
			npcHandler.topic[cid] = 3
		elseif npcHandler.topic[cid] == 3 then
			selfSay("Good, but to make barbarian mead we need some honey which is rare here. I'd hate to waste mead just to learn you're not worth it ...", cid)
