...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'yes') then
		...
		elseif npcHandler.topic[cid] == 8 then
			if player:removeItem(7699, 1) then
				...
				selfSay('You have proven yourself as very efficient. The future may hold great things for you in store ...', cid)
				npcHandler:say('Take this token of gratitude. I hope you can use well what you will find inside!', cid)
			else
	...
	elseif msgcontains(msg, 'join') then
		if player:getStorageValue(Storage.secretService.Quest) < 1 then
			selfSay('Well, well, well! As you might know, we are entrusted by the Venorean tradesmen to ensure the safety of their ventures ...', cid)
			selfSay('This task often puts our representatives in rather dangerous and challenging situations. On the other hand, you can expect a generous compensation for your efforts on our behalf...', cid)
			selfSay('Just keep in mind though that we expect quick action and that we are rather intolerant to needless questions and moral doubts ...', cid)
			npcHandler:say('If you join our ranks, you cannot join the service of another city! So do I understand you correctly, you want to join our small business?', cid)
	...
	elseif msgcontains(msg, 'mission') then
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission01) == 4 and player:getStorageValue(Storage.secretService.Quest) == 3 then
			...
			selfSay('Our Thaian allies are sometimes a bit forgetful. For this reason we are not always informed timely about certain activities. We won\'t insult our great king by pointing out this flaw ...', cid)
			selfSay('Still, we are in dire need of these information so we are forced to take action on our own. Travel to the Thaian castle and \'find\' the documents we need. They have the file name AH-X17L89. ...', cid)
			npcHandler:say('Now go to Thaian castle and get the File.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission02) == 2 and player:getStorageValue(Storage.secretService.Quest) == 5 then
			...
			selfSay('The oppression of Carlin\'s men by their lunatic women is unbearable to some of our authorities. We see it as our honourable duty to support the male resistance in Carlin ...', cid)
			npcHandler:say('The poor guys have some speakeasy in the sewers. Bring them this barrel of beer with our kind regards to strengthen their resistance.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission03) == 4 and player:getStorageValue(Storage.secretService.Quest) == 7 then
			...
			selfSay('As you know, our lovely city is a bastion of civilisation surrounded by numerous hazards. The nearby Plains of Havoc and the hostile elven town Shadowthorn are only a few of the obstacles we have to overcome on an almost daily basis ...', cid)
			selfSay('Against all odds, we managed to gain some modest profit by exploiting these circumstances in one way or the other. Recently though, one of our neighbours went too far...', cid)
			selfSay('In some ruin in the midst of the Green Claw Swamp, a dark knight had fancied himself as the lord of the swamp for quite a while ...', cid)
			selfSay('For some years, we had some sort of gentleman\'s agreement. In exchange for some supplies and luxuries, the deranged knight used his ominous influence over the local bonelord species to supply us with ... certain goods ...', cid)
			selfSay('However, lately the black knight has proven himself to be no gentleman at all. In a fit of unprovoked rage, he slew our emissary and almost all of his henchmen ...', cid)
			npcHandler:say('Even though we can live with this loss, it becomes obvious that the knight\'s madness gets worse which makes him unbearable as a neighbour. Find him in his hideout in the Green Claw Swamp and get rid of him.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission05) == 2 and player:getStorageValue(Storage.secretService.Quest) == 11 then
			...
			selfSay('We try to establish new trade agreements with various potential customers. Sometimes we have to offer some presents in advance to ensure that trade is prospering and flourishing. It will be your task to deliver one of those little presents ...', cid)
			selfSay('The northern barbarians are extremely hostile to us. The ones living in Svargrond are poisoned by the lies of agitators from Carlin. The barbarians that are also known as raiders are another story though ...', cid)
			selfSay('Of course they are extremely wild and hostile but we believe that we will sooner or later profit from it when we are able to improve our relations. Please deliver this chest of weapons to the barbarians as a sign of our good will ...', cid)
			npcHandler:say('Unfortunately, most of them will attack you on sight. It will probably take some time until you find somebody that is willing to talk to you and to accept the weapons.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission06) == 3 and player:getStorageValue(Storage.secretService.Quest) == 13 then
			...
			selfSay('Some dwarven criminal called Blowbeard dares to blackmail our city. He threatens to destroy the whole city and demands an insane amount of gold ...', cid)
			selfSay('Of course we are not willing to give him a single gold coin. It will be your job to get rid of this problem. Go and kill this infamous dwarf ...', cid)
			npcHandler:say('His laboratory is near the technomancer hall. Bring me his beard as proof of his demise.', cid)
			npcHandler.topic[cid] = 0
