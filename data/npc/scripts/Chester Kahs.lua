...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'join') then
		if player:getStorageValue(Storage.secretService.Quest) < 1 then
			selfSay('Our bureau is an old and traditional branch of the Thaian government. It takes more than lip service to join our ranks ...', cid)
			selfSay('Absolute loyalty to the crown and the Thaian cause as well as courage face-to-face with the enemy is the least we expect from our members ...', cid)
			npcHandler:say('You will swear allegiance to Thais alone and abandon the service of any other city. So is it really your wish to become one of our field agents?', cid)
	...
	elseif msgcontains(msg, 'mission') then
		if player:getStorageValue(Storage.secretService.Quest) == 1 and player:getStorageValue(Storage.secretService.AVINMission01) < 1 and player:getStorageValue(Storage.secretService.CGBMission01) < 1 then
			...
			selfSay('Your first task is to deliver a warning. Illegally, the Venoreans are crafting more ships than the Thaian authorities have allowed them ...', cid)
			selfSay('Our sources have told us that those ships often end up in the hands of pirates or smugglers ...', cid)
			selfSay('An official note would strain the relationship between Thais and Venore too much as this would mean that we had to admit officially that we know about those activities ...', cid)
			selfSay('Still, we can\'t allow them to continue like this. It will be your task to let them know that we do not tolerate such behaviour. Get a fire bug from Liberty Bay and set their shipyard on fire ...', cid)
			selfSay('Use the fire bug on some flammable material there to start the fire. It might take a while to find some wood that\'s dry enough for the fire to spread. Just keep trying ... ', cid)
			npcHandler:say('If you get captured or killed during your mission, we will deny any contact with you.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission01) == 3 and player:getStorageValue(Storage.secretService.Quest) == 3 then
			...
			selfSay('Your next mission concerns an internal matter for our agency. Some decades ago, one of our most talented field agents vanished in the Green Claw Swamp ...', cid)
			selfSay('Nowadays, that more and more adventurers are swarming this area, there is an increasing number of reports on some sinister goings-on and mysterious ruins in the middle of the swamp ...', cid)
			selfSay('We got some credible clues that there might be a connection between the ruins and the disappearance of our agent ...', cid)
			selfSay('As he is already missing since decades it is unlikely that he is still alive. Nevertheless, we want you to find out something about the whereabouts of our agent in the ruins in the Green Claw Swamp, north west of Venore ...', cid)
			selfSay('He used to write diaries, maybe you can find one of those, or some other hints, or even his remains. You have to understand that he was a member of a prestigious Thaian family. Very influential people are interested in his whereabouts ...', cid)
			selfSay('The Green Claw Swamp is treacherous and dangerous. You will have a hard time to find any clues ...', cid)
			npcHandler:say('As a small incentive I think its worthy to mention that he was wearing a quite impressive armor. You may keep it for yourself if you stumble across it.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission02) == 2 and player:getStorageValue(Storage.secretService.Quest) == 5 then
			...
			selfSay('One of our agents is missing. He was investigating the cause for the slow growth of our colony Port Hope ...', cid)
			selfSay('You will continue these investigations at the point where the information that the lost agent has sent us ends. Some of the traders in Port Hope must have connections to persons who are interested in sabotaging our efforts in Tiquanda ...', cid)
			npcHandler:say('Search their personal belongings to find some sort of evidence that we could need!', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission03) == 3 and player:getStorageValue(Storage.secretService.Quest) == 7 then
			...
			selfSay('Just recently we were able to secretly help our elven friends to exposure an agitator sent by Carlin to poison our connections with them. The elves\' reaction wasswift and without compromise ...', cid)
			selfSay('They banished the delinquent in a place they call \'Hellgate\'. Unfortunately, we learnt later that the convict was sent there with several of his belongings and it is very likely that he took vital papers with him ...', cid)
			selfSay('These papers can tell us much about Carlin\'s plans in the North. We need you to enter \'Hellgate\' and to retrieve the papers for us ...', cid)
			npcHandler:say('We don\'t care how you get them. Do whatever you think is necessary.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission04) == 2 and player:getStorageValue(Storage.secretService.Quest) == 9 then
			...
			selfSay('It\'s bad enough that Carlin got a solid foothold in the far North but now the Venoreans also try to move in. They try to gain influence on the barbarian raiders by bribing their leaders or making them great promises ...', cid)
			selfSay('We want you to cause some bad blood in this relationship. Travel to their most southern camp, enter the ice tower of their leaders and kill some of them ...', cid)
			npcHandler:say('Here is a signet ring that the Venorean emissaries use to wear. \'Lose\' the ring in the north-western corner of the highest level of the tower. They will surely find it there.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission05) == 3 and player:getStorageValue(Storage.secretService.Quest) == 11 then
			...
			player:addItem(7700, 1)
			selfSay('The women of Carlin have the northern city Svargrond in the firm grip of her manicured hands. At the moment, there is little we can do about it but there is one thing that plays into our hands ...', cid)
			selfSay('The barbarians have surely at least heard about the fact that alcohol is outlawed in Carlin ...', cid)
			npcHandler:say('If some amazonian warrior would smash a beer or ale cask in front of some witnesses, the relationship would surely suffer a bit. So go and disguise yourself as an amazon. Then use a crowbar to destroy a cask.', cid)
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.secretService.TBIMission06) == 3 and player:getStorageValue(Storage.secretService.Quest) == 13 then
			...
			selfSay('Great, you are here. We need your service in a mission of utmost urgency ...', cid)
			selfSay('A mad dwarven technomancer that listens to the name of Blowbeard sent us a blackmailing letter. He demands to deliver all of Thais\'s gold to him. Else he will destroy the city with an artificial earthquake caused by one of his machines! ...', cid)
			npcHandler:say('We need you to find his base in Kazordoon and to kill him before he can use his infernal machine. Bring us his beard as proof of your success.', cid)
			npcHandler.topic[cid] = 0
