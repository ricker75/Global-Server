...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "ankrahmun")) then
		if(npcHandler.topic[cid] == 1) then
			selfSay("Yes, I've lived in Ankrahmun for quite some time. Ahh, good old times! ...", cid) 
			selfSay("Unfortunately I had to relocate. <sigh> ...", cid) 
			npcHandler:say("Business reasons - you know.", cid)
			npcHandler.topic[cid] = 2
		end
	elseif(msgcontains(msg, "supplies")) then
		if(npcHandler.topic[cid] == 2) then
			selfSay("What!? I bet, Baa'leal sent you! ...", cid) 
			npcHandler:say("I won't tell you anything! Shove off!", cid)
