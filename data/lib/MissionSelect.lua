...
Rashid = {
			...
			MissionStart = 200
			MissionEnd	= 210
			WithoutMissionPrice = 1.0 -- newpricebuy = 10000gp / 0.5 = 20000gp // newpricesell = 10000gp * 0.5 = 5000gp
		}
]]
Rashid = {
			NeedMission = false,
...
BlueOrGreen = true		-- blue djinn or green djinn quest, true = only one. -- NOT IMPLEMENTED YET
...
function setNewTradeTable(table)
	...
	for _, v in ipairs(table) do
			items[v.id] = {itemId = v.id, buyPrice = v.buy, sellPrice = v.sell, subType = v.subType or 0, realName = v.name}
	end
	return items
end
		...
		local function onBuy(cid, item, subType, amount, ignoreCap, inBackpacks)
			...
			if items[item].buyPrice <= player:getMoney() then
				if inBackpacks then
					...
					--if not items[item].subType then
					for i = 1, amount do
						container:addItem(items[item].itemId, items[item].subType)
					end
					--end
				else
					if items[item].subType then
						for a = 1, amount do
							doPlayerAddItem(cid, items[item].itemId, items[item].subType or 1)
						end
					else
						player:addItem(items[item].itemId, amount, false, 0)
					end
